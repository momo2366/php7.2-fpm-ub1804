...
RUN apt-get clean && apt-get -y update && apt-get install -y locales curl software-properties-common git tzdata 
RUN add-apt-repository ppa:ondrej/nginx-mainline
RUN apt-get update
RUN apt-get install -y --force-yes --no-install-recommends php7.2-bcmath php7.2-bz2 php7.2-cli php7.2-common php7.2-curl \
...
RUN sed -i "s/upload_max_filesize = .*/upload_max_filesize = 32M/" /etc/php/7.2/fpm/php.ini
RUN sed -i "s/post_max_size = .*/post_max_size = 64M/" /etc/php/7.2/fpm/php.ini
